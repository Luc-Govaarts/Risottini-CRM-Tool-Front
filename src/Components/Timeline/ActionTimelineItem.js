...
export default function TimelineItemLeft(props) {
	...
	const adjusted_due_date = `${moment(adjusted_date).format(
		'YYYY-MM-DD'
	)} ${moment(adjusted_time).format('HH:mm:ss')}`
				...
				<Paper elevation={3}>
					<Box className={classes.action}>
						...
						<Box>
							...
							<Button onClick={handleOpenDelete} color='default' size='small'>
								Verwijderen
							</Button>
						</Box>
					</Box>
					...
					<Dialog open={open_adjust} onClose={handleCloseAdjust}>
						...
						<DialogContent>
							...
							<TextField
								...
								onChange={(e) => {
									set_adjusted_action(e.target.value)
								}}
								margin='dense'
								label='Actie'
								type='text'
							...
							<TextField
								onChange={(e) => {
									set_adjusted_note(e.target.value)
								}}
								value={adjusted_note}
								variant='outlined'
								margin='normal'
						...
						<DialogActions>
							...
							<Button type='submit' color='primary' onClick={handleAdjust}>
								Bevestigen
							</Button>
						</DialogActions>
					</Dialog>
